...
def main():
    ...
    workers = tk.IntVar(root, value=max(1, (os.cpu_count() or 2)-1))
    ...
    def setModel(source):
        classes_local = readclassfile()
        listbox1.delete(0, tk.END)
        listbox2.delete(0, tk.END)
        options_local = classes_local.keys()
        messagebox.showinfo('Model Selection', 'Model %s was selected including %s sound classes.' %(model_weight.get(), len(list(options_local))))
        for item in options_local:
            listbox2.insert(tk.END, item)

    def readclassfile():
        classes = {}
        df = pd.read_csv(f'model/{model_weight.get()}/soundclass.csv', lineterminator='\n', encoding="utf-8")
    ...
    def run():
        ...
        # Run pipeline (multiprocessing handled inside silic2.browser)
        browser(
    ...
    if os.path.isdir('model'):
        for item in os.listdir('model'):
            if os.path.isdir(f'model/{item}'):
                sets.append(item)
                latestmodel = item
    model_weight.set(latestmodel if latestmodel else '')
    opm=tk.OptionMenu(controls, model_weight, *sets, command=setModel)
